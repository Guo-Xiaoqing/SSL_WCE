  ...
  with variable_scope.variable_scope(
    ...
    with arg_scope(
      ...
      with arg_scope([layers.batch_norm], is_training=is_training):
        ...
        net, end_points = resnet_utils.stack_blocks_dense(net, blocks, scope=scope, output_stride=output_stride)
        print(net)
        ...
        img_features = tf.squeeze(net)
        end_points['feature'] = img_features
        # if num_classes is not None:
        #net = layers_lib.conv2d(net, num_classes, [1, 1], activation_fn=None, normalizer_fn=None, scope='logits')
        #net = tf.squeeze(net)
        # Convert end_points_collection into a dictionary of end_points.
        # end_points = utils.convert_collection_to_dict(end_points_collection)
        # if num_classes is not None:
        #   end_points['predictions'] = layers.softmax(net, scope='predictions')
        return img_features, end_points
...
#  ]
  blocks = [
      ...
      resnet_v2_block('block4', base_depth=128, num_units=3, stride=1),
  ]
  return resnet_v2(
      inputs,
      blocks,
...
#  ]
  blocks = [
      resnet_v2_block('block1', base_depth=16, num_units=3, stride=2),
      resnet_v2_block('block2', base_depth=32, num_units=4, stride=2),
      resnet_v2_block('block3', base_depth=64, num_units=23, stride=2),
      resnet_v2_block('block4', base_depth=128, num_units=3, stride=1),
  ]
  return resnet_v2(
      inputs,
